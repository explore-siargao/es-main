import { dbActivities, dbProperties, dbRentals, dbUsers, dbWishlists } from "@repo/database"
...
const seedWishlist = async()=>{
    ...
    try {
        ...
        const flatProperties = getProperties.flatMap(property=>({_id:property._id, category:"Properties"}))
        const getRentals = await dbRentals.find({status:"Live", deletedAt:null})
        const flatRentals = getRentals.flatMap(rental=>({_id:rental._id,category:"Rentals"}))
        const getActivities = await dbActivities.find({status:"Live", deletedAt:null})
        const flatActivities = getActivities.flatMap(activity=>({_id:activity._id, category:"Activities"}))

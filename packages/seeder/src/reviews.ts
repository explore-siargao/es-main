import { dbActivities, dbBookableUnitTypes, dbProperties, dbRentals, dbReviews, dbUsers } from "@repo/database";
import { activityReviewComments, rentalReviewComments, reviewPropertyComments } from "./common/constants/reviews";
...
const seedReview = async () => {
    ...
    try {
        ...
        const getProperties = await dbProperties.find({ status: "Live", deletedAt: null });
        const getRentals = await dbRentals.find({status:"Live", deletedAt:null})
        const getActivities = await dbActivities.find({status:"Live",deletedAt:null})
        ...
        for (const property of getProperties) {
            for (const item of property.bookableUnits) {
                const commentIndex = Math.floor(Math.random() * reviewPropertyComments.length);
        ...
        for (const rental of getRentals){
            const commentIndex = Math.floor(Math.random() * rentalReviewComments.length);
            const userIndex = Math.floor(Math.random() * flatUserIds.length);
            const accuracyRateIndex = Math.floor(Math.random() * rates.length);
            const checkInRateIndex = Math.floor(Math.random() * rates.length);
            const cleanlinessRatesIndex = Math.floor(Math.random() * rates.length);
        ...
        for(const activity of getActivities){
            const commentIndex = Math.floor(Math.random() * activityReviewComments.length);
            const userIndex = Math.floor(Math.random() * flatUserIds.length);
            const accuracyRateIndex = Math.floor(Math.random() * rates.length);
            const checkInRateIndex = Math.floor(Math.random() * rates.length);
            const cleanlinessRatesIndex = Math.floor(Math.random() * rates.length);

...
import { E_Listing_Category, T_Property_Amenity, T_Property_Facility } from "@repo/contract"
...
const FilterPropertyModal: React.FC<T_Props> = ({
  isOpen,
  onClose,
}) => {
  ...
  const defaultPropertyTypes = (propertyTypesSearch && propertyTypesSearch.length > 0 && propertyTypesSearch[0] !== "any" ? propertyTypesSearch
    ?.map(type => propertyTypes.find(({ value }) => value === type)?.value) : propertyTypes[0]?.value) as string[] | "any";
  const defaultFacilities = facilitiesSearch && facilitiesSearch.length > 0 && facilitiesSearch[0] !== "any" ? (facilitiesSearch
    ?.map(item => ({ ...NEW_FACILITIES.find(({ facility }) => facility === item), isSelected: true}))) : [] as T_Property_Facility[];
  const defaultAmenities = amenitiesSearch && amenitiesSearch.length > 0 && amenitiesSearch[0] !== "any" ? (amenitiesSearch
    ?.map(item => ({ ...AMENITIES.find(({ amenity }) => amenity === item), isSelected: true}))) : [] as T_Property_Amenity[];
  ...
  const { data, isLoading } = useGetListingCategoryHighestPrice(E_Listing_Category.Property);
  ...
  const { facilities, setDefaultFacilities } = useSelectFacilityStore((state) => state)
  const { amenities, setDefaultAmenities } = useSelectAmenityStore((state) => state)
  ...
  const handleSubmit = () => {
    ...
    router.push(`${queryString.join("&")}&checkIn=${checkIn ?? "any"}&checkOut=${checkOut ?? "any"}&numberOfGuest=${numberOfGuest ?? "any"}`)
    onClose()
  }
  return (
    <ModalContainer
        ...
        <div className="flex-grow p-6 space-y-6 overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {propertyTypes.map((type) => (
                <div key={`property-type-${type.value}`} className="flex items-center">
                  <Input
                    type="checkbox"
                    id={`property-type-${type.value}`}
                    name="propertyType"
                    value={type.value}
                    checked={state.propertyTypes !== "any" ? state.propertyTypes.some(
                      (t) => t === type.value
          ...
          <div>
            ...
            <div className="space-y-4">
              <CountInput
                label="Bedrooms"
                count={typeof state.bedroomCount === 'number' ? state.bedroomCount : 0}
                setCount={(count) =>
                  dispatch({
              ...
              <CountInput
                label="Bathrooms"
                count={typeof state.bathroomCount === 'number' ? state.bathroomCount : 0}
                setCount={(count) =>
                  dispatch({

...
function Header({
  ...
  return (
      <header className={cn(`w-full inset-x-0 top-0 z-50 bg-white border-y-gray-200/50 border flex flex-col`, isFixed ? "fixed" : "")}>
          ...
          <WidthWrapper width={contentWidth}>
              <nav className="flex items-center justify-between py-2 my-2 w-full" aria-label="Global">
                  <Link href={LINK_HOME} className="-m-1.5 gap-2 flex lg:flex-1 items-center">
                      <Image className="h-12 w-auto" src={Logo} width={500} height={700} alt={APP_NAME} />
                  </Link>
                  <div className="hidden lg:flex lg:flex-1 lg:justify-end space-x-3 items-center relative">
                      <div className="flex gap-2 rounded-full items-center px-2 py-1">
                          <GuidesMenu />
                          <div>
                              {session.role !== E_UserRole.Admin && !session.isHost && !path.includes("/hosting") ? (
                          ...
                          {!session.id && (
                              <div className="flex gap-1 rounded-full items-center px-2 py-1">
                                  <Button variant="primary" size="sm" onClick={() => router.push(LINK_LOGIN)}>
                                      Login | Sign up
                                  </Button>
                              </div>
                          )}

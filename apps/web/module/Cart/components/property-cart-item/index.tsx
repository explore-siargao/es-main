...
function PropertyCartItem({ item, selectedItems, index, toggleCheckbox }: ICartProps) {
    ...
    <div key={propertyItem?._id} className="border rounded-xl p-4 mb-6">
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-4">
          ...
          />
          <img
            src={`/assets/${propertyItem?.photos![0]?.key}`}
            width={140}
            height={140}
            alt="item image"
          ...
          <div className="flex-1">
            ...
            <Typography variant="p" className="text-gray-500">
              {`${propertyItem?.location?.streetAddress}, `}{`${propertyItem?.location?.barangay}, `}{`${propertyItem?.location?.city}`}
            </Typography>
          </div>
        </div>

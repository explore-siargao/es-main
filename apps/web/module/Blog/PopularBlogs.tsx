  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex flex-col gap-4">
        {data.map((item, index) => (
          <Link href={`${WEB_URL}/blogs/${item.blog.slug}`} className="flex gap-4 h-20 items-center" key="Test">
            <div className="relative w-24 h-20">
              <Image
                fill
                style={{ objectFit: "cover" }}
                src={`/assets/${item.blog.hero.images[0]?.image.filename}`}
                alt={item.blog.hero.images[0]?.image.alt || `Blog image ${index}`}
                className="rounded-lg"
              />
            </div>
            <div className="flex-1">
              <Typography variant="h3" className="text-ellipsis">{item.blog.title}</Typography>
            </div>
          </Link>
        ))}
      </div>

...
const HostPolicies = ({ property }: { property: T_Property }) => {
  ...
  const safetyConsiderations = property.policies?.filter((policy) => policy.category === "Safety Considerations")
  const safetyDevices = property.policies?.filter((policy) => policy.category === "Safety Devices")
  const propertyInformation = property.policies?.filter((policy) => policy.category === "Property Information")
  const additionalRules = property.policies?.filter((policy) => policy.category === "Additional Rules")
  return (
    <div>
      ...
      {thingsToKnow && thingsToKnow.length > 0 ? (
        ...
        <div>
          ...
          <ul className="ml-6 mt-4 text-text-400 list-decimal">
            {safetyDevices && safetyDevices.length > 0 ? safetyDevices.map((policy) => (
              <li>
                {policy.policy}
              </li>
            )) : null}
        ...
        <div>
          ...
          <ul className="ml-6 mt-4 text-text-400 list-decimal">
            {propertyInformation && propertyInformation.length > 0 ? propertyInformation.map((policy) => (
              <li>
                {policy.policy}
              </li>
            )) : null}
        ...
        <div>
          ...
          <ul className="ml-6 mt-4 text-text-400 list-decimal">
            {additionalRules && additionalRules.length > 0 ? additionalRules.map((policy) => (
              <li>
                {policy.policy}
              </li>
            )) : null}

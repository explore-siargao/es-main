                              ...
                              >
                                ...
                                &#8369;{
                                  ...
                                  category.pricePerDates?.find((item) => {
                                    const itemFromDate = new Date(item.fromDate).setUTCHours(0, 0, 0, 0);
                                    const itemToDate = new Date(item.toDate).setUTCHours(0, 0, 0, 0);
                                    const currentDate = new Date(date).setUTCHours(0, 0, 0, 0);
                                    
                                    return currentDate >= itemFromDate && currentDate <= itemToDate;
                                  })?.price ?
                                    parseFloat(category.pricePerDates.find(item => {
                                      const itemFromDate = new Date(item.fromDate).setUTCHours(0, 0, 0, 0);
                                      const itemToDate = new Date(item.toDate).setUTCHours(0, 0, 0, 0);
                                      const currentDate = new Date(date).setUTCHours(0, 0, 0, 0);
                                      
                                      return currentDate >= itemFromDate && currentDate <= itemToDate;
                                    }).price.dayRate).toFixed(2) :
                                    parseFloat(`${category.price}`).toFixed(2)
                                  }
                                </div>
                              </div>

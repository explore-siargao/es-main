    ...
    const filterDataByDate = () => {
      ...
      const newFilteredData = {
        items: sampleData?.items?.map((category) => ({
          ...category,
          beds: category.beds.map((bed: Bed) => ({
            ...bed,
            reservations: bed.reservations.filter((reservation) => {
                  ...
                  <React.Fragment key={category.name}>
                    ...
                    {!collapsed[category.name] && (
                      <>
                        {category.rooms?.map(
                          (room: Room, roomIndex: number) => (
                            ...
                            >
                              ...
                              <td
                                colSpan={daysPerPage}
                                className={`border text-center relative ${roomIndex + 1 !== daysPerPage && "border-r-0"}`}
                              >
                        ...
                        {category.beds?.map((bed: Bed, bedIndex: number) => (
                          <tr
                            key={bed.id}
                            className="hover:bg-gray-100 relative"
                          >

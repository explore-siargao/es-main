...
import { T_Calendar_Joiner_Activity, T_Calendar_Reservation, T_Joiner_Slot } from "@repo/contract"
  ...
  return (
    <>
      ...
      {!collapsed[activity.name] &&
        activity.joinerActivities.map(
          (joinerActivity, index) => {
            ...
            return (
              <>
                ...
                >
                  <td className="border py-4 pr-4 pl-12 text-left border-l-0" onClick={() => toggleCollapse(joinerActivity.name)}>
                  ...
                  >
                    <div className="absolute inset-0 z-10 flex h-full">
                      {generateTimeSlotRowBorder({ daysPerPage, startDate, content: price })}
                    </div>
                  </td>
                </tr>
                <ActivityQtySlotRows

...
export const useBedroomStudioStore = create<BedroomStudioStore>((set) => ({
  ...
  deleteBedroomStudio: (index: number) => {
    set((state) => {
      const updatedBedroomsStudio = state.bedroomsStudio.filter((_, idx) => idx !== index);
      console.log("Deleted bedroomStudio at index:", index, "Updated bedroomsStudio:", updatedBedroomsStudio);

...
module.exports = async () => {
  ...
  try {
    const redirectsRes = await fetch(
      `${process.env.PAYLOAD_NEXT_PUBLIC_SERVER_URL}/api/redirects?limit=1000&depth=1`,
    )
    ...
    if (docs) {
      docs.forEach(doc => {
        ...
        let source = from
          .replace(process.env.PAYLOAD_NEXT_PUBLIC_SERVER_URL, '')
          .split('?')[0]
          .toLowerCase()
        ...
        if (type === 'custom' && url) {
          destination = url.replace(process.env.PAYLOAD_NEXT_PUBLIC_SERVER_URL, '')
        }
        ...
        if (
          ...
          destination = `${process.env.PAYLOAD_NEXT_PUBLIC_SERVER_URL}/${

...
import { dbActivities, dbProperties, dbRentals, dbWishlists } from "@repo/database";
...
export const addToWishList = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        const createWishlist = async()=>{
            const existing = await dbWishlists.findOne({userId:userId,listingId:listingId,deletedAt:null})
            if(!existing){
            ...
            res.json(response.success({item:newWishList, message:"Item added to wishlist"}))
        }else{
            res.json(response.error({message:"Item already exist on your wishlist"}))
        }
        }
        if(formattedCategory === "Properties"){
            const checkProperty = await dbProperties.findOne({_id:listingId,deletedAt:null})
            if(!checkProperty){
            ...
            const checkActivity = await dbActivities.findOne({_id:listingId,deletedAt:null})
            if(!checkActivity){
                res.json(response.error({message:"Invalid listingId"}))
            }else{
                createWishlist()
            ...
            const checkRentals = await dbRentals.findOne({_id:listingId,deletedAt:null})
            if(!checkRentals){
                res.json(response.error({message:"Invalid listingId"}))
            }else{
                createWishlist()

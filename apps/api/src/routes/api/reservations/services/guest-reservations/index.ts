...
const fetchPaymentMethodDetails = async (xendItPaymentRequestId: string) => {
  ...
  return {
    ...
    cardDetails: {
      ...
      network:
        paymentRequestData.item.payment_method.card?.card_information?.network,
    },
  }
}
      ...
      const enrichSingleReservationWithPayment = async (reservation: any) => {
        if (referenceId !== 'undefined' && referenceId) {
          const paymentDetails = await fetchPaymentMethodDetails(
            reservation.xendItPaymentRequestId as string
          )
          return {
            ...reservation,
            paymentDetails,
      ...
      if (status === E_ReservationStatus.Cancelled) {
        ...
        if (validReservations.success) {
          if (referenceId === 'undefined') {
            ...
            const detailedReservation =
              await enrichSingleReservationWithPayment(cancelledReservations[0])
            res.json(
              response.success({
        ...
        if (validReservations.success) {
          if (referenceId === 'undefined') {
            ...
            const detailedReservation =
              await enrichSingleReservationWithPayment(finishReservations[0])
            res.json(
              response.success({
                item: detailedReservation,
                message: 'Reservation successfully fetched',
        ...
        if (validReservations.success) {
          if (referenceId === 'undefined') {
            ...
            const detailedReservation =
              await enrichSingleReservationWithPayment(activeReservations[0])
            res.json(
              response.success({
                item: detailedReservation,
                message: 'Reservation successfully fetched',

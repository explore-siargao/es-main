...
import { Z_Add_For_Payment, Z_Update_For_Payment } from "@repo/contract-2/for-payment-listings";
...
export const bookListing = async(req:Request, res:Response)=>{
    ...
    try {
        if((!propertyIds && !rentalIds && !activityIds) || !price || !guestCount || !startDate || !endDate){
            ...
            const newForPayments = new dbForPaymentListing(validForPaymentInput.data)
              const saveForPayment = await newForPayments.save() 
              res.json(response.success({item:saveForPayment, message:"Listing successfully added to for payment"}))
            }else{
...
export const updateForPayment = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(validForPaymentUpdate.success){
            const updateForPayment = await dbForPaymentListing.findByIdAndUpdate(forPaymentId,
                {
                    $set:validForPaymentUpdate.data
                }
            )
            res.json(response.success({item:validForPaymentUpdate.data,message:"Successfullu updated payment details"}))
        }else{

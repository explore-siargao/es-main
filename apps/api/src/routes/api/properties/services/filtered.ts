...
export const getFilteredProperties = async (req: Request, res: Response) => {
  ...
  if (propertyTypes !== 'any' && propertyTypes !== '') {
    propertyTypesInput = String(propertyTypes)
      .split(',')
      .map((item) => item.trim())
  } else {
  ...
  if (validPropertySearch.success) {
    try {
      ...
      if (
            ...
            bookableUnits: item.bookableUnits?.map((item) => ({
              ...
              pricePerDates: item?.pricePerDates?.map((item) => ({
                ...
                price: {
                  ...
                  pricePerAdditionalPerson: !item.price
                    ?.pricePerAdditionalPerson
                    ? 0
                    : convertPrice(
        ...
        const validBookableUnits = Z_Property_Filtered_Result.safeParse(
          paginatedBookableUnits
        )
        if (validBookableUnits.success) {
          ...
          res.json(
            response.error({
              item: paginatedBookableUnits,
              items: JSON.parse(validBookableUnits.error.message),
              message: 'Invalid data exists',
            })
          )
        }
      } else if (
        ...
        const arrayTypes = String(propertyTypes)
          ...
          .split(',')
          .map((item) => item.trim())
        const upperCaseTypes = arrayTypes.map((t: any) =>
